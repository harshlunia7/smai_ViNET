...
#    print(len(ann_maps), len(fix_files), len(frame_files))
   ...
   for map_file, fix_file, frame_file in zip(ann_maps, fix_files, frame_files):
        ...
        os.system(f"mv {fix_file} {os.path.join(data_dir, 'annotations', v, f'fixMap_{str(i).zfill(5)}.mat')}")
        os.system(f"mv {map_file} {os.path.join(data_dir, 'annotations', v, 'maps', f'eyeMap_{str(i).zfill(5)}.jpg')}")
        os.system(f"mv {frame_file} {os.path.join(data_dir, 'video_frames', v, f'img_{str(i).zfill(5)}.jpg')}")
        i += 1
